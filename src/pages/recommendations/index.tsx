...
type ColumnType = ColumnDef<RecommendationInterface, unknown>;
...
export function RecommendationListPage(props: RecommendationListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getRecommendations({
        ...
        order: params.order,
        searchTermKeys: ['content_type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<RecommendationInterface>>(
    () => `/recommendations?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: RecommendationInterface) => {
    if (hasAccess('recommendation', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/recommendations/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'content_type', header: 'Content Type', accessorKey: 'content_type' },
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="recommendation" />
        </Flex>
