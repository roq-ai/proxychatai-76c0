...
import { getConnections } from 'apiSdk/connections';
...
function DashboardPage() {
  ...
  const connectionStat = useSWR<PaginatedInterface<ConnectionInterface>>(
    () => `/connections`,
    () => getConnections({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Connections',
      result: connectionStat?.data?.totalCount,
      isLoading: connectionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Profiles',
